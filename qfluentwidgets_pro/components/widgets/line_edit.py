...
class LineEdit(QLineEdit):
    ...
    def focusedBorderColor(self):
        ...
        return autoFallbackThemeColor(
            self.lightFocusedBorderColor, self.darkFocusedBorderColor
        )
    ...
    def addAction(
        self, action: QAction, position=QLineEdit.ActionPosition.TrailingPosition
    ):
        ...
        if position == QLineEdit.ActionPosition.LeadingPosition:
            self.hBoxLayout.insertWidget(
                len(self.leftButtons), button, 0, Qt.AlignLeading
            )
    ...
    def setCompleterMenu(self, menu):
        ...
        menu.indexActivated.connect(
            lambda idx: self._completer.activated[QModelIndex].emit(idx)
        )
...
class CompleterMenu(RoundMenu):
    ...
    def eventFilter(self, obj, e: QEvent):
        ...
        if e.key() in [Qt.Key_Enter, Qt.Key_Return] and self.view.currentRow() >= 0:
            self._onCompletionItemSelected(
                self.view.currentItem().text(), self.view.currentRow()
            )
            self.close()
    ...
    def popup(self):
        ...
        self.view.setProperty(
            "dropDown", aniType == MenuAnimationType.FADE_IN_DROP_DOWN
        )
