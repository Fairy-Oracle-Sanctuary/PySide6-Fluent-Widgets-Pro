...
class MenuInterface(GalleryInterface):
    ...
    def __init__(self, parent=None):
        t = Translator()
        super().__init__(
            title=t.menus, subtitle="qfluentwidgets.components.widgets", parent=parent
        )
    ...
    def createMenu(self, pos):
        ...
        # add actions
        menu.addActions(
            [Action(FIF.PASTE, self.tr("Paste")), Action(FIF.CANCEL, self.tr("Undo"))]
        )
    ...
    def createCommandBarFlyout(self):
        ...
        view.addHiddenAction(
            Action(FIF.SETTING, self.tr("Settings"), shortcut="Ctrl+S")
        )
